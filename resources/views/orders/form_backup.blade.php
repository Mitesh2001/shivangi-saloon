                        ...
                        <table class="table table-bordered">
							...
							@if(isset($client_products))
								...
								<tr id="products_{{$c_product->product_id}}">
									<td>
										<a class="btn btn-link p-1" onclick="deleteProduct('{{$c_product->product_id}}', '{{$c_product->id}}')"><i class="flaticon2-trash text-danger"></i></a>
										{{ $c_product->product->name }}
										<input type="hidden" class="client_product_id" name="c_products[{{$c_product->product_id}}]" id="c_products_id_{{$c_product->product_id}}" value="{{$c_product->id}}">
										<input type="hidden" class="product product-hidden" name="products[{{$c_product->product_id}}]" id="products_id_{{$c_product->product_id}}" value="{{$c_product->product_id}}">
										<input type="hidden" name="is_new_product[{{$c_product->product_id}}]" id="is_new_product_{{$c_product->product_id}}" value="0">
										<input type="hidden" name="c_product_qty[{{$c_product->product_id}}]" id="c_product_qty_{{$c_product->product_id}}" value="{{ $c_product->qty }}">
										<input type="hidden" class="product-igst-hidden" name="products_igst[{{$c_product->product_id}}]" id="products_igst_{{$c_product->product_id}}" value="{{$c_product->igst}}">
										<input type="hidden" class="product-sgst-hidden" name="products_sgst[{{$c_product->product_id}}]" id="products_sgst_{{$c_product->product_id}}" value="{{$c_product->sgst}}">
										<input type="hidden" class="product-cgst-hidden" name="products_cgst[{{$c_product->product_id}}]" id="products_cgst_{{$c_product->product_id}}" value="{{$c_product->cgst}}">
										@php
									...
									<td class="text-right">
										<input type="number" class="form-control" style="width:80px !important" name="order_qty[{{$c_product->product_id}}]" id="order_qty_{{$c_product->product_id}}" data-addition="{{$c_product->qty}}" onchange="getProductQtyAmount({{$c_product->product_id}},this.value, {{$c_product->qty}})" step="1" value="{{$c_product->qty}}" min="1" number="" required="">
									</td>
									<td class="text-right">
										{{$c_product->product_price}}
										<input type="hidden" name="products_price[{{$c_product->product_id}}]" id="products_price_{{$c_product->product_id}}" value="{{$c_product->product_price}}">
									</td>
									<td>
										<input type="number" class="form-control deal-discount-input text-right" name="deal_discount[{{$c_product->product_id}}]" min="0" max="100" id="deal_discount_{{$c_product->product_id}}" value="{{$c_product->deal_discount}}" readonly="readyonly" onchange="getProductDiscount({{$c_product->product_id}})">
									</td>
									<td>
										<input type="number" class="form-control text-right" name="products_discount[{{$c_product->product_id}}]" min="0" max="100" id="products_discount_{{$c_product->product_id}}" step=".01" onchange="getProductDiscount({{$c_product->product_id}})" value="{{ $c_product->discount }}">
									</td>
									<td class="text-right">
										<span id="discount_amount_{{$c_product->product_id}}">{{$c_product->discount_amount}}</span>
										<input type="hidden" name="products_discount_amount[{{$c_product->product_id}}]" id="products_discount_amount_{{$c_product->product_id}}" value="{{$c_product->discount_amount}}">
									</td>
									<td class="text-right">
										<span id="final_amount_{{$c_product->product_id}}">{{$c_product->final_amount}}</span>
										<input type="hidden" name="products_final_amount[{{$c_product->product_id}}]" id="products_final_amount_{{$c_product->product_id}}" value="{{$c_product->final_amount}}">
									</td>
								</tr>
								@endforeach
							@endif 
