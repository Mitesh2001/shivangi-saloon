...
<div class="card card-custom">
	<div class="card-header">
		...
		@if(\Entrust::can('package-create') && !$allow_view_only)
            <a href="{{ route('package.create') }}" class="btn btn-primary mr-3">Create Package</a>
		@endif
    ...
    <script> 
		$(document).ready(function () { 
			...
			$(document).on('click', '.delete-package', function (e){ 
				...
				let external_id = $(e.target).closest('form').find('.package_id').val(); 
  
				$.ajax({
					url: '{!! route('package.checkdelete') !!}',
					type: "POST",
					dataType: 'json',
					cache: false,
					data: {
