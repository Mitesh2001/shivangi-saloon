...
<div class="card card-custom">
	<div class="card-header">
		...
		<div class="card-toolbar">
			@if(\Entrust::can('holiday-create') && !$allow_view_only)
    ...
    <script> 
		$(document).ready(function () { 
			...
			$(document).on('click', '.delete-holiday', function (e){ 
				...
				let external_id = $(e.target).closest('form').find('.holiday_id').val(); 
  
				$.ajax({
					url: '{!! route('holidays.checkdelete') !!}',
					type: "POST",
					dataType: 'json',
					cache: false,
					data: {
					...
					success: function (data) {
						if(data.status === true) {
							Swal.fire({ 
								text: "Do you want to remove this holiday?",
								icon: 'warning',
