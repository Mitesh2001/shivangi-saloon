...
class ClientsTimelineController extends Controller
{
    ...
    public function ClientsList(Request $request)
    {
        ...
        $data['status_wise'] = 1;
        $data['status'] = $status;
        $data['status_name'] = str_replace('_',' ', $status);
    ...
    {  
        ...
        if($status == "no_risk") {
            $appointments =  $this->getByDate('no_risk', 'No Risk', $distributor_id);
            $clients = $this->getClients($appointments);
        } 

        if($status == "dormant_clients") {
            $appointments =  $this->getDateBetween('dormant_clients', 'Dormant Client', $distributor_id);
            $clients = $this->getClients($appointments);
        } 

        if($status == "at_risk") {
            $appointments =  $this->getDateBetween('at_risk', 'At Risk', $distributor_id);
            $clients = $this->getClients($appointments);
        } 
    ...
    public function getClients($segaments, $client = false) {
    ...
    {    
        ...
        $dormant_clients =  $this->getDateBetween('dormant_clients', 'Dormant Client', $distributor_id);
        $at_risk =  $this->getDateBetween('at_risk', 'At Risk', $distributor_id);
        $no_risk = $this->getByDate('no_risk', 'No Risk', $distributor_id);
        ...
        $arr = [
            ...
            'lost_clients' => count($this->getClients($lost_clients)),
        ]; 
    ...
    public function getByDate($column_name, $status = "", $distributor_id = "")
    {
        ...
        return DB::select("SELECT *, '$status' as status FROM appointments WHERE date BETWEEN CURDATE() - INTERVAL (SELECT other from clients_timelines WHERE name = '$column_name' AND $condition) DAY AND CURDATE() AND distributor_id = $distributor_id AND deleted_at is null");
    }
