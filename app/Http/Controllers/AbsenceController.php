...
class AbsenceController extends Controller
{
    public function indexData()
    {
        if (!auth()->user()->can('absence-view')) {
            session()->flash('success_warning', __('You do not have permission to view this page'));
    ...
    public function store(Request $request)
    {
        ...
        if ($request->user_external_id && auth()->user()->can('absence-manage')) {
            ...
            if (!$user) {
                Session::flash('success_warning', __('Could not find user'));
                return redirect()->back();
            }
        }
        ...
        Session::flash('success', __('Absence registered'));
        return redirect()->back();
    }
    ...
    public function destroy(Absence $absence)
    {
        if (!auth()->user()->can('absence-manage')) {
            Session::flash('success_warning', __('You do not have sufficient privileges for this action'));
            return redirect()->back();
        }
        $absence->delete();
