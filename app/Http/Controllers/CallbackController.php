...
class CallbackController extends Controller
{
    public function dropbox(Request $request)
    {
        ...
        if ($integration) {
            session()->flash('success_warning', __('File integration alredy exists'));
            return redirect()->route('integrations.index');
        }
        if ($request->error) {
            session()->flash('success_warning', __('Access not given, try again'));
            return redirect()->route('integrations.index');
        }
    ...
    public function googleDrive(Request $request)
    {
        ...
        if (!isset($res['refresh_token'])) {
            session()->flash('success_warning', __('It seems you already have a connection to Daybyday. Please remove it in your Google console.'));
            return redirect()->route('integrations.index');
        }
