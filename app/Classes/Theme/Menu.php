...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        // Check if menu is only for system user / owner
        if(isset($item['only_owner_access'])) {
            if(!Helper::is_system_user() && $item['only_owner_access'] = 1) {
                return false;
            }
        } 

        // Check subscription (before show menu)
        if(isset($item['check_subscription'])) {

            $distributor_id =  Helper::getDistributorId(); 
            if($distributor_id != 0 && $item['check_subscription'] == true && Helper::allowViewOnly($distributor_id)) {
                return false;
            } 
        } 
        ...
        if(isset($item['title'])) {
             ...
             // Distributor cant access employeess & branch menu directly
            if(Helper::is_distributor_user() && in_array($item['title'], ['Employees', 'Branch Management'])) {
                return false;
            }

            // System user cant access Branch menu directly
            if(Helper::is_system_user() && in_array($item['title'], ['Branch Management', 'My Commission'])) {
                return false;
            }

            if(Helper::is_system_user() && $item['title'] == "Employees") {
                $item['title'] = "Users";
            }
            if(Helper::is_system_user() && $item['title'] == "All Employees") {
                $item['title'] = "All Users";
            }
            if(Helper::is_system_user() && $item['title'] == "New Employee") {
                $item['title'] = "New User";
            }
        }
